...
UCLIBC_VERSION   := uClibc-${_LIB_VER_}
...
#download tarballs here
	@echo "download src..."
	wget ${DLFLAGS} -O ${_TOP_DIR_}/package/binutils-${_BIN_VER_}.tar.bz2 ${SW_DL_URL}/gnu/binutils/binutils-${_BIN_VER_}.tar.bz2
	wget ${DLFLAGS} -O ${_TOP_DIR_}/package/gcc-${_GCC_VER_}.tar.bz2      ${SW_DL_URL}/gnu/gcc/gcc-${_GCC_VER_}/gcc-${_GCC_VER_}.tar.bz2
	wget ${DLFLAGS} -O ${_TOP_DIR_}/package/gdb-${_GDB_VER_}.tar.bz2      ${SW_DL_URL}/gnu/gdb/gdb-${_GDB_VER_}.tar.bz2

	wget ${DLFLAGS} -O ${_TOP_DIR_}/package/uClibc-${_LIB_VER_}.tar.xz    https://uclibc.org/downloads/uClibc-0.9.33.2.tar.xz
...
ifeq ($(call FILE_DIR_EXIST,binutils-${_BIN_VER_},${PRJROOT}/src),)
	tar -vxf ${_TOP_DIR_}/package/binutils-${_BIN_VER_}.tar.bz2 -C ${PRJROOT}/src
endif
ifeq ($(call FILE_DIR_EXIST,gcc-${_GCC_VER_},${PRJROOT}/src),)
	tar -vxf ${_TOP_DIR_}/package/gcc-${_GCC_VER_}.tar.bz2      -C ${PRJROOT}/src
endif
ifeq ($(call FILE_DIR_EXIST,gdb-${_GDB_VER_},${PRJROOT}/src),)
	tar -vxf ${_TOP_DIR_}/package/gdb-${_GDB_VER_}.tar.bz2      -C ${PRJROOT}/src
endif
ifeq ($(call FILE_DIR_EXIST,linux-${_KNL_VER_},${PRJROOT}/src),)
	tar -Jxvf ${_TOP_DIR_}/package/linux-${_KNL_VER_}.tar.xz    -C ${PRJROOT}/src
endif
ifeq ($(call FILE_DIR_EXIST,uClibc-${_LIB_VER_},${PRJROOT}/src),)
	tar -Jxvf ${_TOP_DIR_}/package/uClibc-${_LIB_VER_}.tar.xz    -C ${PRJROOT}/src
endif
	@echo "unzip src end"
...
ifeq ($(call FILE_DIR_EXIST,Makefile,${_PRJROOT_INN_}/bld/${BINUTILS_VERSION}),)
	${_PRJROOT_INN_}/src/${BINUTILS_VERSION}/configure --target=${TARGET} --prefix=${_PREFIX_INN_} --enable-ld=yes --enable-gold=yes --enable-plugins --with-gnu-ld --with-gnu-as --enable-shared ${_OPTIONS_} 2>&1 |tee configure.out
endif
...
ifeq ($(call FILE_DIR_EXIST,Makefile,${_PRJROOT_INN_}/bld/${GCC_VERSION}-pass1),)
	${_PRJROOT_INN_}/src/${GCC_VERSION}/configure --target=${TARGET} --prefix=${_PREFIX_INN_} --with-mpc=/data/data/com.termux/files/usr --with-mpfr=/data/data/com.termux/files/usr --with-gmp=/data/data/com.termux/files/usr --enable-languages=c --disable-shared --disable-threads --without-headers --disable-libmudflap --disable-nls  2>&1 |tee configure.out
endif
...
ifeq ($(call FILE_DIR_EXIST,Makefile,${_PRJROOT_INN_}/bld/${GCC_VERSION}-pass2),)
	${_PRJROOT_INN_}/src/${GCC_VERSION}/configure --target=${TARGET} --prefix=${_PREFIX_INN_} --with-mpc=/data/data/com.termux/files/usr --with-mpfr=/data/data/com.termux/files/usr --with-gmp=/data/data/com.termux/files/usr --enable-languages=c,c++ --enable-shared --enable-threads=posix --disable-libsanitizer ${_OPTIONS_} --with-headers=${_PREFIX_INN_}/${TARGET}/include --with-libs=${_PREFIX_INN_}/${TARGET}/lib 2>&1 |tee configure.out
endif
	make -w all install 2>&1 |tee make.out
